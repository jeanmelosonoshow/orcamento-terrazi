...
export function normalizarNascimento(valor) {
    if (valor instanceof Date && !Number.isNaN(valor.getTime())) {
        return [
            String(valor.getFullYear()).padStart(4, '0'),
            String(valor.getMonth() + 1).padStart(2, '0'),
            String(valor.getDate()).padStart(2, '0')
        ].join('-');
    }
    const nascimento = String(valor ?? '').trim().slice(0, 10);
    const partes = nascimento.match(/^(\d{4})-(\d{2})-(\d{2})$/);
    if (!partes) return '';
...
export function normalizarArquitetoEntrada(dados = {}) {
    const arquiteto = {
        ...
        nascimento: normalizarNascimento(dados.nascimento),
    ...
    if (!arquiteto.nascimento) throw Object.assign(new Error('Informe uma data de nascimento valida.'), { statusCode: 400 });
    if (arquiteto.nascimento > dataLocalAtual()) throw Object.assign(new Error('A data de nascimento nao pode estar no futuro.'), { statusCode: 400 });
...
export function normalizarArquiteto(linha) {
    ...
    return {
        ...
        nome: linha.nome,
        cpf: linha.cpf,
        nascimento: normalizarNascimento(linha.nascimento),
