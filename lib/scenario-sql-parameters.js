...
function aplicarDiretivasFiltrosOpcionais(sql, filtros = {}) {
    ...
    const diretiva = /\/\*\s*campo\s*:\s*([a-z_][a-z0-9_$]*(?:\s*\.\s*[a-z_][a-z0-9_$]*)*)\s*\|\s*filtro\s*=\s*:(filiais|vendedores)\s*\*\//gi;

    const resultado = texto.replace(diretiva, (_, campoInformado, filtro) => {
        ...
        if (!selecionados.length) return 'AND 1 = 0';
        return `AND ${campo} IN (:${nomeFiltro})`;
    });
    if (/\/\*\s*campo\s*:/i.test(resultado)) {
        throw new Error('Diretiva de filtro opcional invalida. Use /* campo: ALIAS.CAMPO | filtro = :filiais */ ou :vendedores.');
    }
    return resultado;
}
