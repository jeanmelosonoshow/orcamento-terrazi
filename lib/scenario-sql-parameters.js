...
const parametrosCenario = {
    ...
    filiais_permitidas: { tipo: 'VARCHAR(50)', filtro: 'filiaisPermitidas', lista: true },
    filiais: { tipo: 'VARCHAR(50)', filtro: 'filiais', lista: true },
    vendedores: { tipo: 'VARCHAR(50)', filtro: 'vendedores', lista: true },
    arquitetos: { tipo: 'INTEGER', filtro: 'arquitetos', lista: true },
...
function nomeParametroInterno(nome, indice, lista) {
    const loteRelacionamento = nome.match(/^documentos_relacionamento_(\d+)$/);
    if (loteRelacionamento) return `CRM_REL_${loteRelacionamento[1]}_${indice + 1}`;
    const sufixo = lista ? '_' + (indice + 1) : '';
    return 'CRM_SYS_' + nome.toUpperCase() + sufixo;
}
...
function aplicarDiretivasFiltrosOpcionais(sql, filtros = {}) {
    ...
    if (/\/\*\s*(?:operador\b|campo\s*:)/i.test(resultado)) {
        throw new Error('Diretiva de filtro opcional invalida. Use /* operador = AND | campo: ALIAS.CAMPO | filtro = :filiais */; :vendedores ou :arquitetos.');
    }
    return resultado;
}
    ...
    while ((match = regex.exec(mascarado)) !== null) {
        ...
        if (!referencias.has(nome)) {
            ...
            const nomesInternos = itens.map((valor, indice) => {
                const nomeInterno = nomeParametroInterno(nome, indice, configuracao.lista);
...
export function prepararSqlCenario(sql, fonte, filtros = {}) {
    ...
    const mapa = {
        ...
        filiais_permitidas: () => adicionarLista(filtros.filiaisPermitidas),
        filiais: () => adicionarLista(filtros.filiais),
        vendedores: () => adicionarLista(filtros.vendedores),
        arquitetos: () => adicionarLista(filtros.arquitetos),
