...
function criarChaveFiltroRelacionamento({ status, tipos, dataInicial, dataFinal, versao }) {
    const assinatura = JSON.stringify({ status: [...status].sort(), tipos: [...tipos].sort(), dataInicial, dataFinal, versao });
    return `contact-relationship-v2:${crypto.createHash('sha256').update(assinatura).digest('hex')}`;
}

async function consultarFiltroRelacionamento(db, { status, tipos, dataInicial, dataFinal }) {
...
export async function resolverFiltroRelacionamento(db, filtros = {}, opcoes = {}) {
    ...
    const parametros = { status, tipos, dataInicial, dataFinal };
