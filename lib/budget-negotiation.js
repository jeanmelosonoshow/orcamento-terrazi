...
export function normalizarIdFilialSaida(valor) {
    const texto = String(valor ?? '').trim().toUpperCase();
    return texto && texto.length <= 2 ? texto : null;
}

export function normalizarNumeroSaida(valor) {
...
export function normalizarSaidasOrcamento(valor, limite = 50) {
    if (!Array.isArray(valor) || !valor.length || valor.length > limite) return null;
    const chaves = new Set();
    const saidas = [];
    for (const item of valor) {
        const idfilialsaida = normalizarIdFilialSaida(item?.idfilialsaida);
        const numerosaida = normalizarNumeroSaida(item?.numerosaida);
        const chave = `${idfilialsaida || ''}:${numerosaida || ''}`;
        if (!idfilialsaida || !numerosaida || chaves.has(chave)) return null;
        chaves.add(chave);
        saidas.push({ idfilialsaida, numerosaida });
    }
    return saidas;
}

export function normalizarSaidaOrcamento(linha) {
    if (!linha) return null;
    return {
        id: linha.id,
        orcamentoId: linha.orcamento_id,
        statusNegociacaoId: linha.status_negociacao_id,
        idfilialsaida: linha.idfilialsaida,
        numerosaida: linha.numerosaida,
        idfuncionario: linha.idfuncionario,
        idvendedor: linha.idvendedor,
        dataVinculo: linha.data_vinculo
    };
}
...
export async function verificarAcessoOrcamento(cliente, orcamentoId, session) {
    ...
    const resultado = await cliente.query(`
        SELECT o.id
           ...
           AND (
               ...
               OR ($2::TEXT = 'SU' AND EXISTS (
                   SELECT 1 FROM vendedor_orcamento v
                    WHERE v.id_orcamento = o.id AND v.id_filial = ANY($5::TEXT[])
               ))
         ...
         LIMIT 1
    `, [orcamentoId, categoria, idfuncionario, idfilial, filiaisPermitidas]);
