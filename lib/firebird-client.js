...
import { montarSqlLimpezaTabelasTemporarias } from './scenario-sql-validation.js';

const require = createRequire(import.meta.url);
const Firebird = require('node-firebird');
const POOL_STATE_KEY = Symbol.for('terrazi.firebird.pool');
const CHARSET_PRINCIPAL = normalizarCharsetFirebird(process.env.DB_CHARSET_FB, 'NONE');
const CHARSET_ALTERNATIVO = normalizarCharsetFirebird(process.env.DB_CHARSET_FB_FALLBACK, 'UTF8');
...
function classificarErroConexao(error) {
    if (error && !error.code && /connection timeout after/i.test(String(error.message || ''))) {
...
async function adquirirComRetentativa(pool, operacao) {
    ...
    if (ultimoErro?.code === 'FB_ACQUIRE_TIMEOUT') ultimoErro.isFirebirdCapacityError = true;
    else ultimoErro.isFirebirdConnectionError = true;
    throw ultimoErro;
}
...
async function executarConsultaNoCharset(sql, params, opcoes, charset) {
    ...
    try {
        ...
        const sqlLimpeza = montarSqlLimpezaTabelasTemporarias(opcoes.tabelasTemporarias);
        if (sqlLimpeza && !descartarConexao) {
            try {
                await consultar(db, sqlLimpeza, [], Math.min(timeoutMs, 10000));
            } catch (cleanupError) {
                descartarConexao = true;
                console.error('[firebird] falha ao limpar tabelas temporarias; conexao sera descartada', {
                    operacao,
                    tabelas: opcoes.tabelasTemporarias,
                    ...resumirErro(cleanupError)
                });
            }
        }
