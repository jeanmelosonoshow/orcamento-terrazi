...
export function deveTentarCacheBaseVisualizacao(preparado, fonte, visualizacao = {}) {
    return fonte === 'firebird'
        && visualizacao?.agrupar === true
        && visualizacao?.cacheBaseDrill === true
        && !sqlEhExecuteBlock(preparado?.sql);
}

export function prepararConsultaBaseVisualizacao(preparado, limiteLinhas = 3000) {
    const limite = Math.min(10000, Math.max(100, Math.trunc(Number(limiteLinhas) || 3000)));
    return {
        ...preparado,
        sql: `SELECT FIRST ${limite + 1} * FROM (${String(preparado.sql || '').trim().replace(/;\s*$/, '')}) CRM_CACHE_BASE`,
        limiteBaseMemoria: limite
    };
}

export function avaliarBaseVisualizacaoEmMemoria(linhas, limiteLinhas = 3000, limiteBytes = 4194304) {
    const dados = Array.isArray(linhas) ? linhas : [];
    const maximoLinhas = Math.min(10000, Math.max(100, Math.trunc(Number(limiteLinhas) || 3000)));
    const maximoBytes = Math.min(16777216, Math.max(262144, Math.trunc(Number(limiteBytes) || 4194304)));
    const tamanhoBytes = Buffer.byteLength(JSON.stringify(dados), 'utf8');
    return {
        adequada: dados.length <= maximoLinhas && tamanhoBytes <= maximoBytes,
        linhas: dados.length,
        tamanhoBytes,
        excedeuLinhas: dados.length > maximoLinhas,
        excedeuBytes: tamanhoBytes > maximoBytes
    };
}
